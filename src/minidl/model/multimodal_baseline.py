...
@ModelRegistry.register("multimodal_resnet3d_nphase")
class MultiModal_ResNet3D_NPhase(nn.Module):
    """
    ResNet3D-based model for MRI analysis and clinical features using pre-trained weights.
    This model serves as a drop-in replacement for the existing MRI_baseline model,
    but uses a 3D ResNet backbone with optional pre-trained weights.
...
class MultiModal_ResNet3D_NPhase_concat(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        V = torch.cat([V, clinical_feat.unsqueeze(1)], dim=1)  # [B, D * H * W + 1, n * d_model]

        x_avg = F.adaptive_avg_pool1d(V.transpose(1, 2), 1).squeeze(-1)  # [B, n * d_model]
        x_max = F.adaptive_max_pool1d(V.transpose(1, 2), 1).squeeze(-1)  # [B, n * d_model]
...
@ModelRegistry.register("multimodal_resnet3d_nphase_add")
class MultiModal_ResNet3D_NPhase_add(nn.Module):
    """
    ResNet3D-based model for MRI analysis and clinical features using pre-trained weights.
    This model serves as a drop-in replacement for the existing MRI_baseline model,
    but uses a 3D ResNet backbone with optional pre-trained weights.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_max = F.adaptive_max_pool3d(V, 1).squeeze(-1).squeeze(-1).squeeze(-1)  # [B, n * d_model]
...
@ModelRegistry.register("multimodal_resnet3d_nphase_attn_clinical")
class MultiModal_ResNet3D_NPhase_Attn_Clinic(nn.Module):
    """
    ResNet3D-based model for MRI analysis and clinical features using pre-trained weights.
    This model serves as a drop-in replacement for the existing MRI_baseline model,
    but uses a 3D ResNet backbone with optional pre-trained weights.
    ...
    def __init__(
        ...
        self.feed_forward = nn.Sequential(
            nn.Linear(d_model * n_phases, d_model * 4 * n_phases),
            nn.GELU(),
            nn.Linear(d_model * 4 * n_phases, d_model * 4 * n_phases),
            nn.GELU(),
            nn.Linear(d_model * 4 * n_phases, d_model * n_phases),
            nn.GELU(),
        )

        self.attn_norm = nn.LayerNorm((d_model * n_phases))
        self.layer_norm = nn.LayerNorm(d_model * n_phases)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        clinical_q = clinical_feat.unsqueeze(1)  # (B, 1, C_cat)
        attn_output, _ = self.attention(clinical_q, V_pos, V_pos)  # [B, 1, n * d_model]

        attn_output = self.attn_norm(attn_output.squeeze() + clinical_feat)

        x = self.feed_forward(attn_output)  # [B, 2 * n * d_model]
